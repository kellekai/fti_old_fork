...
int FTI_Recover_MPIL4()
{
    ...
    res = MPI_File_open(FTI_Exec.globalComm, fn, MPI_MODE_RDONLY, info, &pfh); 
    
    // check if successfull
    if (res != 0) {
        MPI_Error_string(res, mpi_err, &reslen);
        snprintf(str, FTI_BUFS, "unable to read file [MPI ERROR - %i] %s", res, mpi_err);
        FTI_Print(str, FTI_EROR);
        return FTI_NSCS;
    }
        
    // read data from parallel file on the PFS
    for (i = 0; i < FTI_Exec.nbVar; i++) {
        ...
        //memcpy(FTI_Data[i].ptr, &FTI_Data[i].size, sizeof(long));     
        ...
        res = MPI_File_read_at(pfh, offset, FTI_Data[i].ptr, FTI_Data[i].count, dtype, &status);
           //exit(0); 
       // printf("rank: %i, offset: %i, datasize: %lu\n", FTI_Topo.splitRank, (int)(block_offset + block*(FTI_Topo.splitRank)), FTI_Data[i].size);

        // check if successfull
        if (res != 0) {
            MPI_Error_string(res, mpi_err, &reslen);
            snprintf(str, FTI_BUFS, "unable to load ckpt data [MPI ERROR - %i] %s", res, mpi_err);
            FTI_Print(str, FTI_EROR);
            return FTI_NSCS;
        }
